	...
	if(rotate){
		rot_delay = setTimeout( function(){
			rotation = setInterval(function(){
				rot += 0.02;
				svg_map.attr('transform', 'translate(1000 1000) scale(' + home_scale + ') rotate(' + rot + ')');
			},10)
		}, delay);
	}else{
	...
	switch(lay){
		case 'home':
			...
			setTimeout( function(){
				...
				// $(map).animate({ opacity:0.5}, dur2, in_out);
				...
				cur_scale = 5;
				svg_map.transition().duration(dur2).attr('transform', 'translate(1000 1000) scale(' + home_scale + ') rotate(' + rot + ')');
		...
		case 'list':
			...
			setTimeout( function(){
				...
				$(list).animate({ top: bar_h*2 + search_h }, dur2, in_out);
...
$(intro).on(bt_event, function(){
		set_location('about.html', false, false, true);
})
...
function load(){
	...
	if( cur_page == '' || !get_section || !get_cod ){
			cur_page = 'index.html';
			set_location('index.html', 'home', 'sig', true);
	}
